...
static void Render(Entity& entity)
{
    ...
    for (Entity::Sprite& sprite : entity.sprites)
    {
        ...
        {
            sprite.spriteInstance.position = 100.0f * Tga::Vector2f((float)entity.gridPos.x, (float)entity.gridPos.y) + entity.offset;

            Tga::GraphicsEngine::GetInstance()->GetSpriteDrawer()
...
static void DiscoverScriptsForEntityFromDefinition(Entity& entity, Tga::SceneObjectDefinition* objectDefinition)
{
    ...
    fullPath << Tga::Settings::GameAssetRoot() << "/" << relPath;
    fullPath.NormalizePath();

    std::filesystem::path folder = std::filesystem::path(fullPath.GetData()).parent_path();
...
void GameWorld::Init()
{
    ...
    mySceneDefinitionManager.Init(Tga::Settings::GameAssetRoot().c_str());
}
    ...
    for (auto& pair : scene.GetSceneObjects())
    {
        ...
        for (auto& p : props)
        {
            if (p.type == GetPropertyType<CopyOnWriteWrapper<SceneSprite>>())
            {
                ...
                sprite.name = p.name;
                sprite.spriteShared.texture =
                    GraphicsEngine::GetInstance()->GetTextureManager()
                    .GetTexture(spr.textures[0].GetString());

                sprite.spriteInstance.size = spr.size;
