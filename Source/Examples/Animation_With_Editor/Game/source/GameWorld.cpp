	...
	// this example plays animations directly, not controlled by scripting
	{
		...
		for (const ScenePropertyDefinition& property : objectProperties)
		{
			if (property.name == "clip_idle"_tgaid)
			{
				...
				myPlayerWithAnimationClips.idleAnimationPlayer = ModelFactory::GetInstance().GetAnimationPlayer(myPlayerWithAnimationClips.idleClip.animationSourcePath.GetString(), myPlayerWithAnimationClips.playerModel.GetModel()->GetSkeleton());
			}
			...
			if (property.name == "clip_jog"_tgaid)
			{
				...
				myPlayerWithAnimationClips.jogAnimationPlayer = ModelFactory::GetInstance().GetAnimationPlayer(myPlayerWithAnimationClips.jogClip.animationSourcePath.GetString(), myPlayerWithAnimationClips.playerModel.GetModel()->GetSkeleton());
			}
		}
	}
	...
	// character set up with animation graphs
	{
		...
		if (it != myPlayerWithAnimationScripts.dynamicProperties.end())
		{
			...
			if (poseAndMotion && poseAndMotion->poseGenerator)
			{
				...
				context.skeleton = myPlayerWithAnimationScripts.playerModel.GetModel()->GetSkeleton();
...
{
	auto& engine = *Tga::Application::GetInstance();
	...
	Tga::GraphicsEngine::GetInstance()->GetModelDrawer().DrawLambert(myPlayerWithAnimationClips.playerModel);
	Tga::GraphicsEngine::GetInstance()->GetModelDrawer().DrawLambert(myPlayerWithAnimationScripts.playerModel);
