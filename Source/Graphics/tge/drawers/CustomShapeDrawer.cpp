...
int CustomShapeDrawer::UpdateVertexes(CustomShape& aObject )
{
    ...
    for( SCustomPoint& point : aObject.myPoints )
    {
        ...
        dataVertexPtr[index].x = point.position.x;
        dataVertexPtr[index].y = point.position.y;
        dataVertexPtr[index].z = point.position.z;

        Vector4f linearColor = point.color.AsLinearVec4();
        dataVertexPtr[index].colorR = linearColor.x;
        dataVertexPtr[index].colorG = linearColor.y;
        dataVertexPtr[index].colorB = linearColor.z;
        dataVertexPtr[index].colorA = linearColor.w;

		dataVertexPtr[index].u = point.uv.x;
		dataVertexPtr[index].v = point.uv.y;
