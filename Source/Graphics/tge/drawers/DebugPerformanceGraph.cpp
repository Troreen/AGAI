...
{
	if (!Application::GetInstance()->IsDebugFeatureOn(DebugFeature::FpsGraph))
	{
		return;
	}
	...
	Tga::GraphicsEngine::GetInstance()->GetCustomShapeDrawer().Draw(*myBackground);
	...
	{	
		...
		Tga::GraphicsEngine::GetInstance()->GetLineDrawer().Draw(lines);
