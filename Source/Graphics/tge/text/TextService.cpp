...
Font TextService::GetOrLoad(std::string aFontPathAndName, FontSize aFontSize, unsigned char aBorderSize)
{
	...
	if (!Tga::Settings::ResolveAssetPath(aFontPathAndName, resolvedPath))
	{
		return { nullptr };
	}
	...
	FT_Face face;
	FT_Error error = FT_New_Face(myLibrary, resolvedPath.GetData(), 0, &face);
	if (error != 0)
	{
		return { nullptr };
	}
...
bool TextService::Draw(Tga::Text& aText, Tga::SpriteShader* aCustomShader)
{
	...
	GraphicsStateStack& graphicsStateStack = GraphicsEngine::GetInstance()->GetGraphicsStateStack();
	...
	spriteSharedData.customShader = aCustomShader;
