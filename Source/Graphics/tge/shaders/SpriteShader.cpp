...
bool Tga::SpriteShader::PrepareRender(const SpriteSharedData& aSharedData) const
{
	...
	textures[0] = aSharedData.texture ? aSharedData.texture->GetShaderResourceView() : engine.GetTextureManager().GetWhiteSquareTexture()->GetShaderResourceView();

	for (unsigned short index = 0; index < ShaderMap::MAP_MAX; index++)
	{
		if (aSharedData.maps[index])
		{
			textures[1 + index] = aSharedData.maps[index]->GetShaderResourceView();
		}
		else
		{
	...
	for (int i = 0; i < myCurrentTextureCount; i++)
	{
		...
		customTextures[0] = myBoundTextures[i].texture->GetShaderResourceView();

		context->PSSetShaderResources(myBoundTextures[i].index, 1, customTextures);
	}
