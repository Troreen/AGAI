...
void GraphicsStateStack::BeginFrame()
{
	...
	// Frame buffer is only set once per frame
	{
		...
		dataPtrCommon->totalTime = Application::GetInstance()->GetTotalTime();
		dataPtrCommon->deltaTime = Application::GetInstance()->GetDeltaTime();
...
void GraphicsStateStack::UpdateGpuStates(bool fullReset)
{
	...
	if (fullReset || state.lightDataVersion != myGpuRenderState.lightDataVersion)
	{
		...
		*dataPtrLights = {};
		...
		dataPtrLights->directionalLightToWorldTransform = directionalLight.transform;
		dataPtrLights->directionalWorldToLightTransform = Matrix4x4f::Inverse(directionalLight.transform);
...
bool GraphicsStateStack::CreateRasterizerStates()
{
	...
	result = DX11::Device->CreateRasterizerState(&rasterizerDesc, &myRasterizerStates[(int)RasterizerState::FrontFaceCulling]);
	if (FAILED(result))
		return false;
