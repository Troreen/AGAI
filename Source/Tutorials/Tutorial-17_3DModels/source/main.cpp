...
void Render(RenderData& renderData, GraphicsEngine& graphicsEngine)
{
    ...
    // 3d line rendering
    {
        ...
        {
            Tga::GraphicsEngine::GetInstance()->GetLineDrawer().Draw(line);
        }
    }
    ...
    // Custom shape rendering
    {
        ...
        Tga::GraphicsEngine::GetInstance()->GetCustomShapeDrawer().Draw(renderData.myCustomShape);
    }
...
void Go(void)
{
    ...
    Tga::ApplicationConfiguration& createParameters = Tga::Settings::GetApplicationConfiguration();

    createParameters.winProcCallback = [](HWND, UINT message, WPARAM wParam, LPARAM lParam)
    {
        SInputManager->UpdateEvents(message, wParam, lParam);
        return 0;
    };

    createParameters.applicationName = L"TGE: Tutorial 17";
    createParameters.activateDebugSystems = Tga::DebugFeature::Fps |
    ...
    Tga::GraphicsEngine& graphicsEngine = *Tga::GraphicsEngine::GetInstance();
    {
        ...
        renderData.mySpriteInstanceData.transform.SetPosition({ 0, 300, 0 });
        ...
        Tga::AnimationPlayer animation = modelFactory.GetAnimationPlayer("CH_NPC_Undead_Idle_01_17G3_AN.fbx", animatedModel.GetModel()->GetSkeleton());
        ...
        while (bShouldRun)
        {
...

            {
                ...
                Matrix4x4f animatedModelTransform = animatedModel.GetTransform();
                Matrix4x4f handToWorldTransform = pose.jointTransforms[boneIndex] * animatedModelTransform;
