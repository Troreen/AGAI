...
// We don't need to do this, but in some occations you might see linking done like this
// In these projects linking is configured in premake while generating project files.
...
void Go( void )
{
...

	{
		...
		cfg.windowSize = { static_cast<unsigned short>(windowWidth), static_cast<unsigned short>(windowHeight) };
		cfg.renderSize = { static_cast<unsigned short>(windowWidth), static_cast<unsigned short>(windowHeight) };
	...
	while (true)
	{
		if (!Tga::Application::GetInstance()->BeginFrame())
		{
			break;
		}
