...
void CreatePreviewForAsset(std::string filePath)
{
    ...
    if (hasMesh) 
    {
        ...
        if (animatedModel->IsValid()) //If the model has an animation
        {
            ...
            GlobalRenderData.myAnimation = std::make_shared<Tga::AnimationPlayer>(modelFactory.GetAnimationPlayer(filePath, animatedModel->GetModel()->GetSkeleton()));
    ...
    {
        GlobalRenderData.myAnimation = std::make_shared<Tga::AnimationPlayer>(modelFactory.GetAnimationPlayer(filePath, GlobalRenderData.myAnimatedModel->GetModel()->GetSkeleton()));
    }
...
void Go(void)
{
    ...
    if (Settings::ResolveGameAssetPath("cubemap/", pathStream))
        GlobalLightData.path = std::string(pathStream.GetStringView());

    FilePathStream cubeMapStream;
    if (Settings::ResolveGameAssetPath("cubemap/Gdansk_shipyard_buildings_cubemap.dds", cubeMapStream))
        GlobalLightData.cubeMap = string_cast<std::wstring>(std::string(cubeMapStream.GetStringView()));
    ...
    cfg.applicationName = L"TGA 2D Tutorial 19";
    cfg.activateDebugSystems = Tga::DebugFeature::None;//Tga::DebugFeature::Fps |
    ...
    Tga::GraphicsEngine& graphicsEngine = *Tga::GraphicsEngine::GetInstance();
    {
        ...
        auto aLight = AmbientLight(
            ...
            GraphicsEngine::GetInstance()->GetTextureManager().GetTexture(currentCubeMap, TextureSrgbMode::None)
        );
                    ...
                    for (int n = 0; n < GlobalLightData.s_cubemaps.size(); n++)
                    {
                        ...
                        if (ImGui::Selectable(GlobalLightData.s_cubemaps[n].c_str(), is_selected))
                        {
                            AmbientLight newALight = AmbientLight(
                                Color{ 0.0f, 0.0f, 0.0f },
                                AmbientLightType::Custom,
                                GraphicsEngine::GetInstance()->GetTextureManager().GetTexture(GlobalLightData.s_cubemaps[n].c_str(), TextureSrgbMode::None)
                            );
...
#pragma endregion
            ...
            Application::GetInstance()->SetClearColor(Color(GlobalLightData.rgb[0], GlobalLightData.rgb[1], GlobalLightData.rgb[2]));
...
#pragma endregion
            ...
            ImGui::End();
            ImGui::EndFrame();
            Render(GlobalRenderData, *Tga::GraphicsEngine::GetInstance());
