...
{
	{
		TutorialCommon::Init(L"TGE: Tutorial 15, threading", true, false);
		...
		Tga::SpriteDrawer& spriteDrawer(GraphicsEngine::GetInstance()->GetSpriteDrawer());
		...
		while (true)
		{
			...
			for (SpriteResourceGame& gameSprite : toRenderSprites)
			{
				...
				{
					myEngineSprites[gameSprite.myID].myInstanceData.position = gameSprite.myPosition;
...
void RunLoadingThread()
{
	...
	while (!myShouldExit)
	{
		...
		if (myToLoadResources.size() > 0)
		{
			...
			newEngineResource.mySharedData.texture = Tga::GraphicsEngine::GetInstance()->GetTextureManager().GetTexture(myToLoadResources[0].c_str());
			newEngineResource.myInstanceData.size = resolution.x*0.05f;
			newEngineResource.myInstanceData.pivot = { 0.5f, 0.5f };
